...
class LoginActivity : BaseActivity<LoginViewModel, LayoutLoginBinding>() {
    ...
    override fun observerLiveData() {
        viewModel.stateUserInfor.observe(this) {
            SharePreferenceUtil.addUserType(it.userType, applicationContext)
            when(it.userType) {
                "USER" -> startActivity(Intent(this@LoginActivity, MainActivity::class.java))
                "ADMIN" -> startActivity(Intent(this@LoginActivity, MainAdminActivity::class.java))
                "SELLER" -> startActivity(Intent(this@LoginActivity, MainSellerActivity::class.java))
            }
            finishAffinity()
        }
    }
